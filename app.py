...
st.title("Elementary Analysis of Acquired Real Estate Data ")
col1, col2,  = st.columns((10,10))




px.set_mapbox_access_token("pk.eyJ1IjoiZG5vdXJpMTgiLCJhIjoiY2w0bnZraTVtMDE1dTNibDZ3cmNjcGphZSJ9.wttGjVxiFNcQQcfbWxaw8Q")
...
print(center1)
print()
zoomFun1, centerFun1 = plotlyFunctions.get_plotting_zoom_level_and_center_coordinates_from_lonlat_tuples(longitudes=lon, latitudes= lat)
print(zoomFun1)
print(centerFun1)
...
figCol1 = px.scatter_mapbox(dfState1, lat="latitude", lon="longitude",color="unformattedPrice", size="unformattedPrice",
                  color_continuous_scale=px.colors.sequential.Plasma, title=state1, zoom=zoomFun1, center=cen1)
#%%
...
corFig = plotlyFunctions.makeHeatmap(dfState1[['unformattedPrice','zestimate','taxAssessedValue','addressZipcode','beds', 'baths', 'area','latitude', 'longitude']])
col1.plotly_chart(corFig, height=800, use_container_width=True)


columnnsList = df[['unformattedPrice','zestimate','taxAssessedValue', 'addressZipcode', 'beds', 'baths', 'area','latitude', 'longitude']].columns.tolist()
...
zoomFun2, centerFun2 = plotlyFunctions.get_plotting_zoom_level_and_center_coordinates_from_lonlat_tuples(longitudes=lon2, latitudes= lat2)
cen2 = {'lon': centerFun2[0], 'lat': centerFun2[1]}

figCol2 = px.scatter_mapbox(dfState2, lat="latitude", lon="longitude",color="unformattedPrice", size="unformattedPrice",
                  color_continuous_scale=px.colors.sequential.Plasma, title=state2, zoom=zoomFun2, center=cen2)
#%%
#st.header("Wealth Concentrations by Property Listing Prices Per US State")
...
corFig2 = plotlyFunctions.makeHeatmap(dfState2[['unformattedPrice','zestimate','taxAssessedValue','addressZipcode','beds', 'baths', 'area','latitude', 'longitude']])
col2.plotly_chart(corFig2, height=800, use_container_width=True)


columnnsList2 = df[['unformattedPrice','zestimate','taxAssessedValue', 'addressZipcode', 'beds', 'baths', 'area','latitude', 'longitude']].columns.tolist()
